...
export function buildGui(app: App): GUI {
  ...
  const palettes: CrystalPaletteName[] = [
    "Amethyst",
    "Ice",
    "Emerald",
    "Citrine",
  ...
  fFissure
    .add(f, "width", 0.02, 0.16)
    .name("Crack width")
    .onChange(liveFissure);
  ...
  const auroraPalettes: AuroraPaletteName[] = [
    "Borealis",
    "Twilight",
    "Ember",
    "Spectrum",
  ...
  fAurora
    .add(a, "palette", auroraPalettes)
    .name("Palette")
    .onChange(liveAurora);
  fAurora
    .add(a, "height", 0.15, 1.3)
    .name("Curtain height")
    .onChange(liveAurora);
  ...
  function syncFolders(m: ModeName): void {
    ...
    for (const g of fissureFolders)
      m === "Molten fissures" ? g.show() : g.hide();
    for (const g of auroraFolders) m === "Aurora silk" ? g.show() : g.hide();
    for (const g of reefFolders)
      m === "Bioluminescent reef" ? g.show() : g.hide();
  }
  syncFolders(s.mode);
